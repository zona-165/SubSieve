...
| 选项卡 | 功能 |
|--------|------|
| 安全状态 | 集中显示今日请求/IP/Token 指纹、拦截量、名单数量、12 项防护机制、Token 拉取限制、缓存/规则/日志健康、最近处理和风险复核队列；可配置观察阈值 |
...
| IP白名单 | 增删、导入白名单 IP/CIDR 并立即生效；显式可信基础设施可在这里维护 |
| IP黑名单 | 增删黑名单 IPv4/CIDR（nginx deny 403），增删后立即生效 |
| Token黑名单 | 精确封禁指定订阅 Token，命中订阅入口后返回 403，并从风险统计中排除避免重复告警 |
| UA规则 | 分别维护 UA 黑名单和白名单，大小写不敏感，立即 reload nginx 生效 |
| 系统设置 | 修改界面、后台凭证、机场上游、订阅路径和网关端口；管理 Webhook / Telegram 高危告警 |

### 安全状态与观察规则

安全状态页默认采用**只观察、人工复核**模式。后台每分钟预热 `guard_cache.json`，从最近日志中计算以下规则：
