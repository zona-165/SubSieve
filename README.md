...
| 风险分析 | 统一风险工作台，按高频拉取、Token 异常、来源异常和脚本扫描分类；集中提供状态筛选、人工复核、分页、日志联动、Token 调查和 IP 封禁；可调用管理员配置的 AI 对整个队列或单条风险生成证据、误报因素、复核建议和拟议动作，但不会自动执行处置；云厂商 CIDR 的真实 403 拦截会记录命中厂商、ASN、规则、请求路径、UA 与触发次数 |
