...
告警推送默认关闭，可在后台「系统设置 → 告警推送」中开启。当前支持 Webhook 和 Telegram；后台维护任务每分钟读取统计缓存，发现脚本/扫描器拉取订阅、可疑 IP、多 IP 拉取同一 Token 等高危事件时推送。同一事件 1 小时内只推送一次，去重状态保存在 `/etc/nginx/subscribe/alert_state.json`，最近检查状态和推送历史保存在 `/etc/nginx/subscribe/alert_history.json`。建议先使用「测试推送」确认渠道可用，再开启正式告警。
