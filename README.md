...
告警推送默认关闭，可在后台「系统设置 → 告警推送」中开启。当前支持 Webhook 和 Telegram；后台维护任务每分钟读取统计缓存，发现脚本/扫描器拉取订阅、可疑 IP、多 IP 拉取同一 Token 等高危事件时推送。告警阈值可在后台调整，包括扫描器评分、可疑 IP 评分、Token 被多少个 IP 拉取才告警、同一事件去重分钟数、历史保留条数；也可一键套用「严格 / 均衡 / 安静」预设。可开启静默时段，静默期间命中的事件会写入历史并计入去重，但不会外发 Webhook / Telegram，后台会显示最近静默摘要、历史状态分布和历史覆盖时间。去重状态保存在 `/etc/nginx/subscribe/alert_state.json`，最近检查状态和推送历史保存在 `/etc/nginx/subscribe/alert_history.json`，最近记录可选择每页显示 10 / 25 / 50 条，支持分页查看，并可显示当前页记录范围，也可快速跳到首页、末页或指定页码；支持按状态和时间范围筛选，支持按 IP、Token 片段、错误原因等关键词搜索，搜索输入会自动防抖并显示等待状态，可点击搜索按钮或按 Enter 立即搜索，关键词可一键清空，也可按 Esc 清空；搜索区在手机端会自动换行，避免按钮挤压输入框，当前状态、时间、每页条数和关键词会以筛选摘要标签显示，点击标签可快速恢复默认或清空关键词；筛选条件可一键重置；记录会同时显示精确时间和相对时间，单条记录、当前页和当前页摘要均可复制文本报告，复制文本和当前页 JSON 导出都会包含当前筛选条件和页码范围；可删除单条展示记录，也可导出当前页或全量历史 JSON 留档；移动端单条记录会将操作按钮下沉为更易点击的双按钮布局；导入前会预览条数、状态分布和时间范围，确认后只替换展示历史，不修改告警配置和去重状态。建议先使用「测试推送」确认渠道可用；需要立刻验证当前统计是否会触发告警时，可点击「立即检查」手动执行一次巡检；如需重新推送同一事件，可在告警状态区点击「重置去重」。
...
- 2026-07-13：告警最近记录搜索新增防抖，减少输入过程中的重复刷新。
- 2026-07-13：告警最近记录搜索新增等待状态提示，输入防抖期间反馈更明确。
- 2026-07-13：告警最近记录搜索新增清空按钮，可快速移除关键词并刷新结果。
- 2026-07-13：告警最近记录搜索框支持 Esc 清空关键词。
- 2026-07-13：告警最近记录搜索框支持 Enter 立即搜索，跳过防抖等待。
- 2026-07-13：告警最近记录搜索新增显式搜索按钮，移动端也能立即触发搜索。
- 2026-07-13：告警最近记录搜索区优化手机排版，窄屏下输入框和按钮自动分行。
- 2026-07-13：告警最近记录列表优化手机排版，单条记录操作按钮在窄屏下更易点击。
- 2026-07-13：告警最近记录新增筛选摘要标签，当前状态、时间、每页条数和关键词更直观。
- 2026-07-13：告警最近记录筛选摘要标签支持点击恢复默认或清空关键词。
- 2026-07-13：告警最近记录标题显示当前页记录范围，翻页时更容易定位。
- 2026-07-13：告警最近记录复制当前页时带上筛选条件和页码范围，便于留档。
- 2026-07-13：告警最近记录导出当前页 JSON 增加筛选上下文和页码范围。
