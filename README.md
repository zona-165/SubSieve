...
| 选项卡 | 功能 |
|--------|------|
| 运行总览 | 集中显示今日请求/IP/Token 指纹、拦截量、名单数量、防护机制、缓存/规则/日志健康和最近处理 |
...
| 风险分析 | 统一风险工作台，按高频拉取、Token 异常、来源异常和脚本扫描分类；集中提供状态筛选、人工复核、分页、日志联动、Token 调查和 IP 封禁，IP 风险证据包含请求状态分布及已有多源情报 |
| 防护策略 | 集中设置 Token 24 小时独立 IP、每分钟拉取、暂停时长和自动暂停开关；当前用量区分 24 小时历史与本次规则周期，并可打开 Token 调查档案；同时维护仅观察的行为阈值与严格/均衡/宽松预设 |
| 访问控制 | 在同一页面切换 IP 白名单、IP 黑名单、Token 黑名单和 UA 放行/拦截规则，保留原有导入导出与即时生效机制 |
| 系统设置 | 按“界面与账户 / 网关与上游 / 运维与通知”分组管理后台凭证、机场反代、订阅路径、端口、证书、统计缓存和告警 |
...
4. **云厂商 IP**：自动识别Ucloud、阿里云、腾讯云、字节、华为云、Google、AWS、Azure、DigitalOcean 等，返回 403
