...
| 选项卡 | 功能 |
|--------|------|
| 安全状态 | 集中显示今日请求/IP/Token 指纹、拦截量、名单数量、11 项防护机制、缓存/规则/日志健康、最近处理和风险复核队列；可配置观察阈值 |
...
3. **UA 白名单**：只豁免上一项 UA 规则，不能绕过云厂商 IP 或 Token 黑名单
4. **云厂商 IP**：自动识别Ucloud、阿里云、腾讯云、字节、华为云、Google、AWS、Azure、DigitalOcean 等，返回 403
5. **Token 黑名单**：命中封禁 Token 返回 403
6. **IP 白名单**：跳过云 IP、UA 和 Token 风险拦截；显式 IP 黑名单和速率限制仍然生效
7. **速率限制**：每分钟 20 次，burst 5，超出返回 429
...
- 2026-08-04：后台重构为安全运营面板，新增安全状态首页、五类可配置观察阈值、Token 指纹化风险队列、人工复核状态、11 项机制状态、运行健康和最近处理；保持独立网关边界，不接入机场数据库。
