...
| 选项卡 | 功能 |
|--------|------|
| 运行总览 | 集中显示今日请求/IP/Token 指纹、拦截量、名单数量、防护机制、缓存/规则/日志健康、最近处理和风险复核队列 |
...
| 风险分析 | 仅保留可疑 Token、可疑 IP、脚本/扫描器和用户画像四个核心处置入口；IP 情报包含多源一致性与置信度 |
...
运行总览中的风险复核默认采用**只观察、人工复核**模式；阈值统一在「防护策略」中配置。后台每分钟预热 `guard_cache.json`，从最近日志中计算以下规则：
...
风险项可标记为“待复核 / 持续观察 / 判定可信 / 确认异常”，判断保存在 `guard_reviews.json`。人工状态本身不会封禁；只有管理员明确点击封禁或进入名单页面添加规则才会改变网关行为。安全状态接口只返回 HMAC 指纹化 Token，不在新增的总览和复核数据中返回 Token 明文。
